...
class StarshipRevisionDeleteForm extends ConfirmFormBase {
  ...
  public static function create(ContainerInterface $container) {
    ...
    return new static(
      $entity_manager->getStorage('starship'),
  ...
  public function getFormId() {
    return 'starship_revision_delete_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $form_state->setRedirect(
      'entity.starship.canonical',
       ['starship' => $this->revision->id()]
    );
    if ($this->connection->query('SELECT COUNT(DISTINCT vid) FROM {starship_field_revision} WHERE id = :id', [':id' => $this->revision->id()])->fetchField() > 1) {
      $form_state->setRedirect(
