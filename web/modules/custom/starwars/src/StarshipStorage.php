...
class StarshipStorage extends SqlContentEntityStorage implements StarshipStorageInterface {
  ...
  public function revisionIds(StarshipInterface $entity) {
    return $this->database->query(
      'SELECT vid FROM {starship_revision} WHERE id=:id ORDER BY vid',
      [':id' => $entity->id()]
    )->fetchCol();
  }
  ...
  public function userRevisionIds(AccountInterface $account) {
    return $this->database->query(
      'SELECT vid FROM {starship_field_revision} WHERE uid = :uid ORDER BY vid',
      [':uid' => $account->id()]
    )->fetchCol();
  }
  ...
  public function countDefaultLanguageRevisions(StarshipInterface $entity) {
    return $this->database->query('SELECT COUNT(*) FROM {starship_field_revision} WHERE id = :id AND default_langcode = 1', [':id' => $entity->id()])
      ->fetchField();
  }
  ...
  public function clearRevisionsLanguage(LanguageInterface $language) {
    return $this->database->update('starship_revision')
